...
def discover(api_key):
    ...
    return {"streams": [a.to_catalog_dict()
                        for a in metric_streams + full_streams if a.schema.get("properties")]}
...
def get_schema_from_api_call(stream, endpoint, api_key, klavio_endpoint_id=None):
    ...
    r = authed_get(stream, url, {'api_key': api_key, 'count': 100, 'sort': 'desc'})
    observations = []
    schema = {}
    for rec in json.loads(r.text).get("data"):
        try:
            schema = add_observations(acc=schema, path=observations, data=rec)
        except Exception as e:
            print(f"WARNING: error parsing schema for {stream}: {e}")
    j_schema = to_json_schema(schema)
    return cleanup_schema(j_schema)
...
def cleanup_schema(schema):
    ...
    for field_name in schema.get('properties', {}).keys():
        if "properties" in schema["properties"][field_name]:
            schema["properties"][field_name] = cleanup_schema(schema["properties"][field_name])
        elif "items" in schema["properties"][field_name]:
            schema["properties"][field_name]["items"] = cleanup_schema(schema["properties"][field_name]["items"])
        else:
            if schema["properties"][field_name].get("format", "") in format_corrects.keys():
                schema["properties"][field_name]["type"] = ["null", format_corrects[schema["properties"][field_name]["format"]]]
                schema["properties"][field_name].pop("format")
    return schema
...
def merge_schemas(schema_truth, schema_new):
    ...
    pd = {"string": 1, "integer": 2, "number": 3, "boolean": 3, "object": -1, "array": 0, "null": 1836}
